...
export class SimpleNodeCsv {
  static appendFile(path, obj){
    let arrayJson = [];

    if (fse.pathExistsSync(path)) {
      arrayJson = SimpleNodeCsv.csvfile2obj(path);
    }

    arrayJson.push(obj);
    SimpleNodeCsv.obj2csvfile(arrayJson, path)
  }
  static csvfile2obj(path){
    return SimpleNodeCsv.csv2obj(fse.readFileSync(path, "utf-8"), ",");
  }
  static obj2csvfile(arrayJson, path){
    const arrayCsv = SimpleNodeCsv.obj2csv(arrayJson, ",");
    fse.outputFileSync(path, arrayCsv);
  }
    ...
    for (rownum = 1; rownum < lines.length; rownum++) {
      ...
      for (collumNum = 0; collumNum < headers.length; collumNum++) {
        if (
          ...
          ret[rownum - 1][headers[collumNum]] = row[collumNum].replace(
            /(^\s*"*|"*\s*$)/g,
            ""
          );
      }
